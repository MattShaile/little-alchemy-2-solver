...
function showNextItem() {
    ...
    const nextItem = craftOrder.find((recipe) => {
        const item = recipe[0];
        if (!item.complete) {
            return recipe;
        }
    ...
    if (nextItem) {
        nextDiv.innerHTML = drawCombo(nextItem[0], nextItem[1]);
        nextDiv.onclick = nextItem[0].row.onclick;
    } else {
        nextDiv.innerHTML = "🎉🎈🎆🌟✨💖 Well done! 💖✨🌟🎆🎈🎉";
...
function drawStats() {
    ...
    let complete = available.reduce((total, item) => total + (item.complete ? 1 : 0), 0);
    // Account for time automatically unlocking
    if (complete > 99) {
        complete++;
    }
    ...
    element.innerHTML = `${complete} crafted`;
    element.innerHTML += "<br>";
    element.innerHTML += `${available.length - complete} remaining`;
}

function drawList() {
    craftOrder.forEach((recipe) => {
        const item = recipe[0];
        const combination = recipe[1];

        const row = document.createElement("tr");
        const cell = document.createElement("td");
        cell.innerHTML = drawCombo(item, combination);
        row.appendChild(cell);

        row.onclick = () => {
            item.complete = !item.complete;
            row.style.backgroundColor = item.complete ? "#7bc86c" : "#ffffff";
...
function drawCombo(item, combination) {
    return `${combination[0].name} ➕ ${combination[1].name} ➡️ ${item.name}`;
}
...
function generateBuildOrder() {
    ...
    while (unavailable.length) {
        ...
        const itemToCraft = unavailable.find((item) => {
            const ingredients = item.combinations.find((combination) => {
                if (available.indexOf(combination[0]) > -1 && available.indexOf(combination[1]) > -1) {
                    return combination;
                }
            });
        ...
        if (!itemToCraft) {
            throw new Error("No item to craft!");
        }

        craft(itemToCraft, craftIngredients);
        ...
        shortCircuit++;
        if (shortCircuit > 1000) {
            throw new Error("Something went wrong: " + unavailable.length + " items remaining");
        }
    }
}

function craft(item, combination) {
    available.push(item);
    unavailable.splice(unavailable.indexOf(item), 1);

    if (combination) {
        craftOrder.push([item, combination]);
    }
}
...
function processRawData() {
    ...
    rows.forEach((row) => {
        ...
        item.name = rowData[0].toLowerCase();
        item.combinationData = rowData[1];
    ...
    items.forEach((item) => {
        ...
        if (item.combinationData === "Available from start.") {
            craft(item);
            item.complete = true;
        } else if (item.combinationData === "Unlocked through progression.") {
            // 100 combos to unlock
            timeItem = item;
        } else {
            const combinationStrings = item.combinationData.split(" / ");
            combinationStrings.forEach((combinationString) => {
                const combinationNames = combinationString.split(", ");

                const partAName = combinationNames[0].toLowerCase();
                const partBName = combinationNames[1].toLowerCase();

                const partA = itemMap[partAName];
                const partB = itemMap[partBName];

                if (!partA) {
                    throw new Error("Unknown item A: " + partAName);
                }
                if (!partB) {
                    throw new Error("Unknown item B: " + partBName);
                }

                item.combinations.push([partA, partB]);
            });
...
const rawData = `Acid Rain	rain, smoke / rain, smog / rain, sickness / cloud, smoke / cloud, smog / cloud, sickness / rain, city
...
Cup	smoothie, container / tea, container / milk shake, container / smoothie, bottle / tea, bottle / milk shake, bottle
...
Frankenstein’s Monster	corpse, legend / corpse, lightning / corpse, electricity / corpse, story / zombie, electricity / zombie, lightning
...
Grim Reaper	human, scythe / corpse, scythe / zombie, scythe / scythe, death / boat, scythe / scythe, river
